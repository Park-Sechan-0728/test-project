...
def get_script_organize(script):
    with st.spinner('강의를 요약중입니다.'):
        try : 
            res = openai.ChatCompletion.create(
                model="gpt-3.5-turbo-0613",
                messages=[
                    {"role":"system","content":"""
                     From now on, you will be the one organizing this content. When the user inputs the lecture script, the input data must be organized according to the instructions below.
                     instructions : 
                     output
                     - 제목
                     - 목차
            ...
            print(e)
            st.error("OpenAI API로부터 강의 요약본을 얻는 동안 오류가 발생했습니다.")
            return None
        return script
...
st.title("📖 강의 스크립트 요약 서비스")

# Create a form for the user to input the child's information.
with st.form(key='my_form'):
    st.write("강의 스크립트를 입력해 주세요")
    script = st.text_input(label='스크립트')
    submit_button = st.form_submit_button(label='강의 요약하기')

# When the form is submitted, check the fields and display the story.
if submit_button:
    if not script:
        st.error("스크립트가 입력되지 않았습니다.")
    else:
